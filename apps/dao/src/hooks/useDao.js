// src/hooks/useDao.js
// (ported from hrpdaolens/src/hooks/useLensDAO.js with no logic changes —
// the hook was already fully generic, with no Lens specifics in it)
...
export const CONTRACTS = {
  ...
  // ⚠️ FIX: this deployment actually accepts TWO tokens in TipJar —
  // mintable tUSD AND real Sepolia USDC (see .env.local's
  // VITE_TEST_TOKEN_USDC) — but testTokenUSDC was missing from this
  // object entirely, even though the env var was already set and
  // CardPreview.jsx already read it directly via import.meta.env for
  // its tip-token buttons. Any code going through CONTRACTS (like
  // TreasuryPage.jsx) had no way to see the USDC address, which is why
  // the Treasury page only ever showed tUSD balances. testTokenGHO is
  // kept as an empty slot for the future (if the DAO adds a THIRD
  // token via TipJar.setAcceptedToken()) — code that reads
  // CONTRACTS.testTokenGHO must check for an empty string itself, not
  // assume the token exists.
  testTokenUSD: import.meta.env.VITE_TEST_TOKEN_USD || "",
  testTokenUSDC: import.meta.env.VITE_TEST_TOKEN_USDC || "",
...
// independent of whatever RPC the wallet itself is configured to use
...
// where it's recalculated every block. Every call site now passes
...
// transaction. See Dossier's useLensDAO.js for the original report
// and full explanation (identical hook, identical fix).
  ...
  if (feeProvider) {
    try {
      ...
      if (feeData.maxFeePerGas != null && feeData.maxPriorityFeePerGas != null) {
        ...
        // AFTER the user reviews and confirms the wallet popup, which
        // can easily take several seconds. Arbitrum Sepolia's baseFee
        // is small in absolute terms, so even a routine block-to-block
        // move can be a large percentage swing — a fee that was +50%
        // above baseFee at fetch time can end up BELOW the (new)
        // baseFee by the time the tx actually lands, which is exactly
        // the "max fee per gas less than block base fee" error this
        // caused in practice (see Dossier's identical fix in
        // useLensDAO.js for the original report). Since this is
        // testnet gas — genuinely fractions of a cent even at many
        // multiples — there's no real cost to being generous: +300%
        // (4x) instead of +50% absorbs several consecutive
        // fee-spiking blocks' worth of margin, not just one.
        // maxPriorityFeePerGas is also floored to a small non-zero
        // value: some RPCs report exactly 0n (legal, but a 0-tip
        // transaction can be the first thing deprioritized under any
        // load), and 0n obviously provides no headroom of its own the
        // way maxFeePerGas's multiplier does.
        const priorityFee =
...
const RANKING_ABI = [
  ...
  // ── Added for the seat-proof flow (claimGlobalSeat / proposeScoped
  // seat args) — GEO REFORM v14/v15. Without these, the frontend has
  // no way to fetch (seatIndex, seatProof) once a level/EARTH
  // overflows past NODE_CAPACITY=244, and proposeScoped()/
  // claimGlobalSeat() start reverting with "not in ... top-244 seat
  // set" for everyone with no way to recover in the UI.
  "function lastSubmittedEpoch() external view returns (uint256)",
  "function lastSubmittedAt() external view returns (uint256)",
  "function epochSeatRoot(uint256 epoch) external view returns (bytes32)",
  "function epochLeafCount(uint256 epoch) external view returns (uint256)",
  "function isRootStale(uint256 maxStaleness) external view returns (bool)",
  "function verifySeatLeaf(uint256 epoch, uint256 index, address account, bool hasSeat, int8 level, uint64 branchId, bytes32[] proof) external view returns (bool)",
  "event SeatRootSubmitted(uint256 indexed epoch, bytes32 root, uint256 leafCount)",
];

// A standalone Interface (never attached to a Contract instance) used
// ONLY to decode the calldata of a past submitSeatAssignments() tx —
// the four arrays passed to it are NOT stored on-chain (only the
// resulting Merkle root + leafCount are), so reconstructing anyone's
// (index, seatProof) means finding the exact tx that emitted
// SeatRootSubmitted for the target epoch and decoding ITS calldata.
const SEAT_SUBMIT_IFACE = new ethers.Interface([
  "function submitSeatAssignments(address[] accounts, bool[] hasSeatFlags, int8[] levels, uint64[] branchIds)",
]);
...
const GOV_ABI_BASE = [
  ...
  // ── Seat-proof flow (see RANKING_ABI comment above for the full
  // rationale) — lets the frontend actually call claimGlobalSeat()
  // and read the staleness window used to decide whether proving a
  // seat is even necessary right now.
  "function claimGlobalSeat(uint256 seatIndex, bytes32[] seatProof) external",
  "function maxRootStaleness() external view returns (uint256)",
  "function verifiedAsOfEpoch(address account) external view returns (uint256)",
...
export function useDao() {
  ...
  //    linked through Privy, e.g. MetaMask/WalletConnect) →
  //    Arbitrum Sepolia ──
  ...
  // entirely separate wallet connection alongside the one Privy
  // already manages — two parallel, mutually unaware paths both
  // talking to the same extension, racing for the same
  // one-request-at-a-time slot. That's the actual root cause behind
  // repeated "already pending" collisions and inconsistent wallet
  // state chased individually over several previous fixes (see
  // Dossier's useLensDAO.js for the original report — identical hook,
  // identical fix). There is now exactly ONE way this app ever
  // connects to a wallet: through Privy (embedded wallet OR
  // MetaMask/WalletConnect/other wallets linked THROUGH Privy). If
  // wagmi doesn't have an active connector yet, there is no wallet to
  // fall back to — connect() below opens Privy's own login modal
  ...
  // someone using this app has already gone through Privy's login),
  // that call was pure redundancy — the wallet is already authorized,
  // nothing to request. If NO wallet is connected yet, this now opens
  // PRIVY's own login modal (email/Google/passkey/wallet) instead of
  // demanding MetaMask specifically — once that resolves,
  // usePrivyWalletSync populates wagmi's connector, and the retry
  // effect watching dao.silentConnect (see Layout.jsx) picks the rest
  // up automatically.
  ...
  const silentConnect = useCallback(async () => {
    ...
    if (!provider) {
      console.warn("[silentConnect] no provider (no active wagmi connector yet)");
      return { success: false };
    }
    try {
      ...
      if (!accounts || accounts.length === 0) {
        console.warn("[silentConnect] eth_accounts returned none — wallet not authorized yet");
        return { success: false };
      }
      ...
      try {
        ...
        // Most likely culprit for "works on the 2nd try, or when
        // reconnecting a wallet already on the right chain, but not on
        // a fresh connect": wallet_switchEthereumChain/addEthereumChain
        // can pop up an approval prompt in the extension. If the user
        // doesn't click it fast enough (or doesn't notice it), this
        // throws/rejects and used to disappear into a bare
        // {success:false} — indistinguishable from any other failure.
        console.warn("[silentConnect] chain switch/add failed:", chainErr);
        return { success: false };
      }
      ...
      console.warn("[silentConnect] failed:", err);
      return { success: false };
    }
  }, [connector]);
  ...
  const disconnect = useCallback(() => {
    ...
    setError(null);
    // ⚠️ FIXED (reconnecting after "Change wallet"/disconnect reloaded
    // the page mid-sync, before the new connection ever finished):
    // connect()/silentConnect() both register accountsChanged/
    // chainChanged listeners that reload the page — but disconnect()
    // never removed them. MetaMask fires accountsChanged with the
    // newly-authorized account as a NORMAL part of reconnecting (not
    // just on revoke) — so the STALE listener from the wallet's
    // previous, already-ended session caught that event and reloaded
    // the page right as usePrivyWalletSync was mid-sync, cutting off
    // useGuardedConnect's own retry loop before it ever got a chance to
    // succeed and register a fresh listener of its own. Every
    // disconnect+reconnect cycle repeated this, so a fresh connection
    // could never actually complete via this page. Remove both
    // listeners here, before anything else touches the provider.
    _getProvider()
      .then((provider) => {
        provider?.removeAllListeners?.("accountsChanged");
        provider?.removeAllListeners?.("chainChanged");
        // Attempt to revoke the site's permission in MetaMask itself
        // (EIP-2255, not supported by all providers) — not critical if
        // it fails.
  ...
  // ── Seat-proof flow (GEO REFORM v14/v15) ─────────────────
  //
  // Once a level (EARTH or a specific hexagon) crosses NODE_CAPACITY
  // (244) candidates, DaoGovernor._propose()/_validateScopeChoice()
  // start requiring a Merkle proof that the caller is in that level's
  // published top-244 seat set (CouncilRankingEpoch.submitSeatAssignments,
  // EPOCH_SUBMITTER_ROLE). The four arrays that went into building the
  // root are NOT stored on-chain — only the resulting root + leafCount
  // are (epochSeatRoot/epochLeafCount) — so reconstructing anyone's
  // own (index, proof) means: find the tx that published the current
  // epoch's root (SeatRootSubmitted), decode ITS calldata to recover
  // the arrays, rebuild the same tree client-side (SeatMerkleLib.sol —
  // sorted-pair commutativeKeccak256, odd node carried up unchanged),
  // and extract the sibling-hash path for the caller's own leaf.
  //
  // ⚠️ This only covers the submitSeatAssignments() path. If the DAO
  // later also deploys OptimisticEpochSubmission and its permissionless
  // submitSeatRoot() path gets used instead, the arrays live in THAT
  // contract's own dispute-window calldata, not here — this function
  // would need a second lookup branch for that case (not yet needed:
  // OptimisticEpochSubmission isn't deployed in this environment).

  /// Local re-implementation of SeatMerkleLib.leafHash — MUST stay
  /// byte-for-byte identical to the Solidity version, or every proof
  /// built here will simply fail verifySeatLeaf()/claimGlobalSeat().
  function _seatLeafHash(index, account, hasSeat, level, branchId) {
    const encoded = ethers.AbiCoder.defaultAbiCoder().encode(
      ["uint256", "address", "bool", "int8", "uint64"],
      [index, account, hasSeat, level, branchId],
    );
    return ethers.keccak256(encoded);
  }

  /// Local re-implementation of OpenZeppelin's Hashes.commutativeKeccak256
  /// (sorted-pair hash) — same rule MerkleProof.verify uses, and the
  /// one SeatMerkleLib.buildRoot() calls directly.
  function _commutativeKeccak256(a, b) {
    const [x, y] = BigInt(a) < BigInt(b) ? [a, b] : [b, a];
    return ethers.keccak256(ethers.concat([x, y]));
  }

  /// Rebuilds the same bottom-up tree SeatMerkleLib.buildRoot() does,
  /// returning the sibling-hash proof array for `targetIndex`. An odd
  /// node with no pair at some level is carried up UNCHANGED and
  /// contributes NOTHING to the proof at that level (matches the
  /// Solidity `else { next[i] = level[left]; }` branch exactly).
  function _buildSeatProof(leaves, targetIndex) {
    ...
    while (level.length > 1) {
      ...
      for (let i = 0; i < level.length; i += 2) {
        next.push(
          i + 1 < level.length
            ? _commutativeKeccak256(level[i], level[i + 1])
            : level[i],
        );
      }
  ...
  /// Returns { needed, found, seatIndex, seatProof, epoch } for the
  /// given (level, branchId) and the currently connected account.
  /// `needed: false` means proposeScoped()/propose() will NOT enforce
  /// the seat check right now (node not overflowed, or the submitter
  /// has gone stale past maxRootStaleness — the contract's own
  /// dead-man's-switch) — passing (0, []) is safe exactly like before.
  /// `needed: true, found: false` means the account genuinely isn't in
  /// the published top-244 for this level — proposeScoped() WILL
  /// revert, and no proof can fix that (the UI should say so plainly
  /// rather than let the transaction fail silently later).
  const getSeatProofStatus = useCallback(
    async (level, branchId) => {
      if (!rankingReadRef.current || !daoGovReadRef.current || !account) {
        return { needed: false, found: false, seatIndex: 0, seatProof: [] };
      }
      try {
        const key = await rankingReadRef.current.nodeKeyOf(level, branchId);
        const overflowed = await rankingReadRef.current.nodeOverflowed(key);
        if (!overflowed) {
          return { needed: false, found: false, seatIndex: 0, seatProof: [] };
        }

        const maxStaleness = await daoGovReadRef.current.maxRootStaleness();
        const stale = await rankingReadRef.current.isRootStale(maxStaleness);
        if (stale) {
          return { needed: false, found: false, seatIndex: 0, seatProof: [] };
        }

        const epoch = await rankingReadRef.current.lastSubmittedEpoch();
        if (epoch === 0n) {
          return {
            needed: true,
            found: false,
            seatIndex: 0,
            seatProof: [],
            error: "No seat data has been published yet for this epoch.",
          };
        }

        // Find the exact tx that published this epoch's root, to
        // recover the arrays that went into it.
        const currentBlock = await readRef.current.getBlockNumber();
        const fromBlock = _proposalsFromBlock(currentBlock);
        const topicHash = ethers.id(
          "SeatRootSubmitted(uint256,bytes32,uint256)",
        );
        const epochTopic = ethers.zeroPadValue(ethers.toBeHex(epoch), 32);
        const logs = await _getLogsAdaptive(
          readRef.current,
          {
            address: CONTRACTS.councilRankingEpoch,
            topics: [topicHash, epochTopic],
          },
          fromBlock,
          currentBlock,
        );
        if (logs.length === 0) {
          return {
            ...
            seatProof: [],
            error:
              "Couldn't locate the on-chain transaction that published this epoch's seat root.",
          };
        }
        // If submitted more than once for the same epoch (shouldn't
        // normally happen), the most recent one wins — matches how
        // epochSeatRoot[epoch] itself just gets overwritten on-chain.
        const log = logs[logs.length - 1];
        const tx = await readRef.current.getTransaction(log.transactionHash);
        if (!tx) {
          return {
            needed: true,
            found: false,
            seatIndex: 0,
            seatProof: [],
            error: "Couldn't fetch the seat-submission transaction.",
          };
        }

        let decoded;
        try {
          decoded = SEAT_SUBMIT_IFACE.decodeFunctionData(
          ...
          return {
            ...
            seatProof: [],
            error:
              "This epoch's root wasn't published via submitSeatAssignments() — can't reconstruct the tree from this transaction (OptimisticEpochSubmission path isn't supported yet).",
          };
        }
        const [accounts, hasSeatFlags, levels, branchIds] = decoded;

        const leaves = accounts.map((acc, i) =>
          _seatLeafHash(i, acc, hasSeatFlags[i], levels[i], branchIds[i]),
        );
        ...
        const myIndex = accounts.findIndex(
          (acc, i) =>
            ...
            hasSeatFlags[i] === true &&
            Number(levels[i]) === Number(level) &&
            BigInt(branchIds[i]) === BigInt(branchId),
        );
        if (myIndex === -1) {
          return {
            needed: true,
            ...
            epoch,
            error:
              "You're not in this level's published top-244 seat set for the current epoch.",
          };
        }

        const seatProof = _buildSeatProof(leaves, myIndex);

        // Sanity-check locally before ever sending a transaction —
        // cheap (pure view call) and turns a wasted on-chain revert
        // into an immediate, clear error instead.
        const verifiedOnChain = await rankingReadRef.current.verifySeatLeaf(
          epoch,
          myIndex,
          account,
          true,
        ...
        if (!verifiedOnChain) {
          return {
            ...
            epoch,
            error:
              "Locally-built proof failed on-chain verification — the seat data may have been republished since. Try again after refreshing.",
          };
        }

        return { needed: true, found: true, seatIndex: myIndex, seatProof, epoch };
      } catch (err) {
        return {
          needed: true,
          found: false,
          seatIndex: 0,
          seatProof: [],
          error: err?.message?.slice(0, 160) || "Unknown error building seat proof.",
        };
      }
    },
    [account],
  ...
  /// Explicit one-time-per-epoch "claim my EARTH seat" action —
  /// DaoGovernor.claimGlobalSeat() (see DaoGovernor.sol). Needed
  /// separately from proposeScoped()'s own inline seat args because
  /// plain propose()/_propose() gate on verifiedAsOfEpoch, which is
  /// only ever set by actually calling this.
  const claimGlobalSeat = useCallback(
    async (onProgress) => {
      const gov = _gov(true);
      if (!gov) return { success: false, error: "not_connected" };
      try {
        onProgress?.("Checking your EARTH seat proof...");
        const status = await getSeatProofStatus(LEVEL_EARTH, EARTH_BRANCH);
        if (!status.needed) {
          return {
            success: false,
            error:
              "EARTH isn't currently overflowed (or the submitter has gone stale) — claiming a seat isn't necessary right now.",
          };
        }
        if (!status.found) {
          return {
            success: false,
            error: status.error || "You're not in the published EARTH seat set.",
          };
        }
        onProgress?.("Sending claimGlobalSeat()...");
        const gasOpts = await _estimateGasWithBuffer(
          gov.claimGlobalSeat,
          [status.seatIndex, status.seatProof],
          GAS_OPTS,
          (signerRef.current?.provider || readRef.current),
        );
        const tx = await gov.claimGlobalSeat(
          status.seatIndex,
          status.seatProof,
          gasOpts,
        );
        onProgress?.(`TX: ${tx.hash.slice(0, 20)}...`);
        await tx.wait();
        return { success: true, epoch: status.epoch };
      } catch (err) {
        return {
          success: false,
          error: err?.reason || err?.info?.error?.message || err.message,
        };
      }
    },
    [getSeatProofStatus],
  ...
  const submitProposal = useCallback(
    async ({ title, description, type, params, scopeChoice }, onProgress) => {
      ...
      if (type === "test") {
        ...
        // Multi-action support: params.actions is an array of
        // { target, value, calldata } executed atomically in one
        // proposal (all-or-nothing) — the same underlying mechanism
        // Governor.propose() always had (targets[]/values[]/calldatas[]);
        // the single-target shape below is kept only for backward
        // compatibility with any older caller still using it.
        if (Array.isArray(params?.actions) && params.actions.length > 0) {
          const invalidIdx = params.actions.findIndex(
            (a) => !a?.target || !ethers.isAddress(a.target),
          );
          if (invalidIdx !== -1) {
            return {
              success: false,
              error: `Action ${invalidIdx + 1}: invalid target address.`,
            };
          }
          targets = params.actions.map((a) => a.target);
          values = params.actions.map((a) => {
            try {
          ...
          calldatas = params.actions.map((a) => a.calldata || "0x");
      ...
      try {
        ...
        // scopeChoice).
        //
        // ⚠️ GEO REFORM v14/v15 ("fair ranking"): DaoGovernor requires a
        // Merkle proof of seat, BUT only when this specific level is
        // ACTUALLY overflowed (nodeOverflowed) — NODE_CAPACITY=244
        // candidates. getSeatProofStatus() checks that first and
        // returns (0, []) untouched whenever it isn't needed, exactly
        // matching the previous safe placeholder behaviour — the only
        // change is that it now ALSO fetches a real proof on the rare
        // path where one actually is required, instead of leaving
        // every overflowed-level proposal permanently broken.
        const seatLevel = scopeLevel;
        const seatBranchId = scopeBranch;
        onProgress?.("Checking seat requirements...");
        const seatStatus = await getSeatProofStatus(seatLevel, seatBranchId);
        if (seatStatus.needed && !seatStatus.found) {
          return {
            success: false,
            error:
              seatStatus.error ||
              "This level is full (244 seats) and you're not in the published set — a proposal can't be submitted here right now.",
          };
        }
        const seatIndex = seatStatus.seatIndex;
        const seatProof = seatStatus.seatProof;
    ...
    [hasCouncil, myEffectiveHex, getSeatProofStatus],
  );
