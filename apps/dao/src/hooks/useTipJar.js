...
const TIPJAR_ABI = [
  ...
  // ⚠️ Genesis-fairness bootstrap (see Treasury.sol/TipJar.sol NatSpec):
  // if Treasury didn't have TIPJAR_ROLE yet at tip() time, the pool share
  // stays escrowed HERE in TipJar instead of reaching Treasury.
  // sweepEscrowToTreasury() is permissionless — anyone can call it once
  // Treasury has the role, to push the accumulated balance over.
  "function escrowedPool(address token) external view returns (uint256)",
...
export function useTipJar() {
  ...
  // ── Genesis-fairness escrow (see TIPJAR_ABI comment above) ──────

  /**
   * @returns bigint — raw escrowed amount (token units, not yet formatted
   *          with decimals) still sitting in TipJar for this token,
   *          waiting to be swept into Treasury.
   */
  const getEscrowedPool = useCallback(
    async (tokenAddress) => {
      _ensureRead();
      try {
        const addr = tokenAddress || DEFAULT_TIP_TOKEN;
        return await tipJarReadRef.current.escrowedPool(addr);
      } catch (e) {
        console.warn("[getEscrowedPool] failed:", e);
        return 0n;
      }
    },
    [_ensureRead],
  ...
  /**
   * Push the accumulated escrow for one token from TipJar into Treasury.
   * Permissionless on-chain (sweepEscrowToTreasury has no role check) —
   * any connected wallet can call this, it doesn't have to be the
   * original tipper(s).
   */
  const sweepEscrow = useCallback(
    async (tokenAddress) => {
      if (!tokenAddress) return { success: false, error: "Token address missing" };
      setLoading(true);
      setError(null);
      setProgress("");
      try {
        const { tipJarWrite } = await _getWriteContracts();

        // Re-check right before sending — sweepEscrowToTreasury() reverts
        // on a zero amount, and the on-screen figure may be stale by the
        // time the button is pressed (e.g. someone else already swept it).
        const pending = await tipJarWrite.escrowedPool(tokenAddress);
        if (pending === 0n) {
          return { success: false, error: "Nothing to sweep for this token" };
        }

        setProgress("Estimating gas...");
        let gasOpts = GAS_OPTS;
        try {
          const estimatedGas = await tipJarWrite.sweepEscrowToTreasury.estimateGas(tokenAddress);
          gasOpts = { gasLimit: (estimatedGas * 130n) / 100n };
        } catch (estimateErr) {
          console.warn("[sweepEscrow] estimateGas failed, fallback to static GAS_OPTS:", estimateErr);
        }

        setProgress("Sweeping escrow into Treasury...");
        const tx = await tipJarWrite.sweepEscrowToTreasury(tokenAddress, gasOpts);
        setProgress(`TX: ${tx.hash.slice(0, 20)}... waiting for block`);
        const receipt = await tx.wait();

        if (receipt.status === 1) {
          setProgress("✓ Escrow swept!");
          return { success: true, txHash: tx.hash };
        }
