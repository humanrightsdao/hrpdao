...
// Same worker as AtticusChat.jsx's /api/chat routes (worker/index.js
// handles both) — same-origin relative call in production, with the
// same optional override for local dev against a different/deployed
// backend. ⚠️ Unlike AtticusChat.jsx, this file had NO such override
// before: it hardcoded the relative path, so on `npm run dev` (plain
// `vite`, no wrangler, no /api proxy in vite.config.js) every request
// 404s, checkOnboardingStatus's catch block "fails open" (returns
// true — deliberately, so a real network hiccup never traps a genuine
// member behind a modal they can't get past), and the overlay silently
// never appears — for ANY wallet, new or not. Set
// VITE_ATTICUS_API_URL in .env.local to a deployed worker URL (or run
// `wrangler dev` + a matching /api proxy in vite.config.js) to actually
// exercise this flow locally.
...
export async function checkOnboardingStatus(addr) {
  if (!addr) return true;
  try {
    const res = await fetch(`${API_BASE}/api/onboarding/status?address=${encodeURIComponent(addr)}`);
    if (!res.ok) {
      console.warn(`⚠️ Onboarding status check returned ${res.status} — failing open.`);
      return true;
    }
...
export default function OnboardingOverlay({ account, onDone, onDisconnect }) {
    ...
    <div className="fixed inset-0 z-50 bg-ink/95 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
      <div className="w-full max-w-xl rounded-3xl border border-hairline bg-surface p-8 sm:p-10 relative my-8">
        ...
        <div className="relative flex items-center justify-between gap-2 mb-2">
          {/* Escape hatch: this overlay is `fixed inset-0 z-50`, rendered
              on top of Layout's own sidebar (z-40) — which is where the
              normal "Вийти" button lives. Without this, a wallet that
              lands here (fresh wallet, or someone who just doesn't want
              to onboard right now) has no way to disconnect and try a
              different one; they're stuck until they finish the lessons
              and test. Reuses Layout's own handleDisconnect via prop
              rather than re-implementing the disconnect sequence here. */}
          {onDisconnect ? (
            <button
              type="button"
              onClick={onDisconnect}
              className="flex items-center gap-1.5 font-mono text-xs px-3 py-2 border border-hairline rounded-full text-parchmentDim hover:border-seal hover:text-sealBright transition-colors"
