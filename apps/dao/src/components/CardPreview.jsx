    ...
    <div className="relative">
      ...
      {status && (
        <>
          ...
          {TIP_TOKENS.length === 0 ? (
            ...
            <div className="space-y-3">
              ...
              {/* "Not enough balance" is already shown on the Send button
                  ...
                  separate paragraph here just repeated the same fact. */}
              ...
              {viewerAddress && (
                <p className="text-center font-mono text-[10px] text-parchmentDim -mb-1">
                  ...
                  <button
                    ...
                    className="underline decoration-dotted underline-offset-2 hover:text-verdigrisBright transition-colors"
                  >
                    {t("dao.card.changeWallet")}
                  </button>
                </p>
              )}
              {isSelf ? (
                ...
                >
                  ...
                  {dao.connecting
                    ? t("dao.card.connecting")
                    : t("dao.card.connectToSend")}
                </button>
              ) : (
                <button
                  type="button"
          ...
          {showFooterNote && (
            <p className="font-mono text-[10px] text-parchmentDim/60 mt-4 text-center">
              {t("dao.card.footerNote", { chain: ACTIVE_CHAIN?.name })}
            </p>
          )}

          {/* Small, easy-to-ignore invite for a genuine outside visitor —
              never shown to the card owner viewing their own card
              (isSelf), and deliberately styled to sit well below the tip
              form so it never competes with the "Send" button. */}
          {!isSelf && (
            <p className="font-mono text-[11px] text-center mt-2">
              <Link
                to="/"
                className="text-parchmentDim/70 hover:text-verdigrisBright transition-colors"
              >
                {t("dao.card.joinInvite")}
              </Link>
            </p>
          )}
        </>
