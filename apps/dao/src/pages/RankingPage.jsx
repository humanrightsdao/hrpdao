...
import { Trophy, Globe2, Layers, Users, KeyRound } from "lucide-react";
...
export default function RankingPage() {
  ...
  async function handleClaimSeat() {
    setClaiming(true);
    setClaimMsg(t("dao.ranking.claimSeatCheckFirst"));
    const res = await dao.claimGlobalSeat((p) => setClaimMsg(p));
    if (res.success) {
      setClaimMsg(t("dao.ranking.claimSeatSuccess", { epoch: res.epoch?.toString?.() ?? res.epoch }));
    } else {
      setClaimMsg(res.error || t("dao.ranking.claimSeatNotNeeded"));
    }
    setClaiming(false);
  }
    ...
    <div className="fade-rise max-w-2xl">
      ...
      {!o ? (
        ...
        <>
          ...
          {dao.isConnected && o.earthOverflowed && dao.hasCouncil && (
            <div className="rounded-2xl border border-hairline p-6 sm:p-8 mb-6">
              <div className="font-mono text-[12px] uppercase tracking-widest text-parchmentDim mb-2 flex items-center gap-2">
                <KeyRound size={14} />
                {t("dao.ranking.claimSeatTitle")}
              </div>
              <p className="text-parchmentDim text-sm leading-relaxed mb-4">
                {t("dao.ranking.claimSeatExplanation")}
              </p>
              <button
              ...
              >
                {claiming ? t("dao.ranking.claimSeatClaiming") : t("dao.ranking.claimSeatButton")}
              </button>
              {claimMsg && (
                <p className="font-mono text-[12px] text-parchmentDim/80 mt-3">{claimMsg}</p>
              )}
            </div>
          )}
        </>
