...
//
// ⚠️ COPIED HERE ONLY so hooks/useLensDAO.js's declareLocation()/
// revealLocationLevel() (ported wholesale from dao-app's useDao.js —
// see that file's header) has something to dynamically import.
// Nothing in THIS app's UI calls those two functions or this file —
// dossier used to have its own "geo" tab (LocationSettings.jsx,
// EpochRankingSection.jsx, both now deleted) with a separate, simpler,
// non-ZK location declaration flow, no commitment/proof. That tab was
// removed since the DAO app already has its own Location/Ranking
// pages on the same contracts. Vite statically resolves dynamic
// import() paths at transform time even when the function is never
// called at runtime, so without this file useLensDAO.js failed to
// even load ("Failed to resolve import '../lib/locationZk'"), which
// broke every other file that imports the hook too. If dossier ever
// grows its own ZK-privacy location flow, wire it up for real instead
// of relying on this copy staying in sync with dao-app's.
// ---------------------------------------------------------------------------
