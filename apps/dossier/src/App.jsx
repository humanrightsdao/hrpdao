...
import PublicFeedPreview from "./components/PublicFeedPreview";
...
function App() {
  ...
  // === Login screen ===
  return (
    <div className="min-h-screen flex items-start justify-center bg-gradient-to-br from-gray-50 to-blue-50 dark:from-gray-900 dark:to-gray-800 px-4 py-10">
      <div className="w-full max-w-6xl">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-0 items-start">
      <div className="w-full lg:sticky lg:top-0 lg:h-screen lg:flex lg:flex-col relative">
      {/* Manual language override, pinned to the top-right corner of
          this column. Small/muted on purpose — automatic (browser-
          based) language detection is untouched and is what most
          people will rely on; this is just a low-key secondary way to
          pick a different language by hand. */}
      <div className="absolute top-0 right-2 z-10 scale-90 origin-top-right">
        <LanguageSelector
          compact
          buttonClassName="flex items-center gap-1 rounded-full border border-slate-300 dark:border-white/10 bg-transparent text-slate-500 dark:text-white/30 hover:text-slate-800 dark:hover:text-white/60 hover:border-slate-400 dark:hover:border-white/20 transition-colors"
        />
      </div>

      {/* Top spacer: grows/shrinks so the card below (and the Log in
          button near its top) lands at the exact vertical midpoint of
          the screen, instead of the logo+card+footer being centered
          as one lump (which pushed the button off-center whenever the
          footer below was a different height than the logo above). */}
      <div className="lg:flex-1 lg:flex lg:flex-col lg:justify-end w-full">
      <div className="w-full max-w-lg mx-auto lg:mx-0 pt-12 lg:pt-0">
        {/* Logo */}
        ...
        <div className="bg-white dark:bg-[#000d1f] border border-slate-300 dark:border-white/[0.07] rounded-xl p-6 space-y-4">
          ...
          {!isConnected && (
            <div className="space-y-3">
              ...
              >
                {walletReady
                  ? t("log_in_button") || "Log in"
                  : t("loading") || "Loading..."}
              </button>

              <div className="text-center text-[11px] text-slate-600 dark:text-white/40">
                {t("login_screen_description") ||
                  "Sign in with email, Google, or passkey — no browser extension required. Prefer your own wallet? You can connect one from the same login screen."}
              </div>
            </div>
          )}
      ...
      {/* Bottom spacer, mirrors the top one: keeps the footer pinned
          just below the card without it (or the empty space beneath
          it) skewing the card away from the exact vertical midpoint. */}
      <div className="lg:flex-1 lg:flex lg:flex-col lg:justify-start w-full">
      <div className="w-full max-w-lg mx-auto lg:mx-0">
        {/* Same About/Terms/Privacy footer as RightSidebar.jsx (the
            in-app sidebar shown once logged in) — reused as-is here so
            it's reachable before login too. Lives inside this sticky
            column (not after the whole page) specifically so it stays
            in view next to the login card instead of ending up buried
            under however many posts the feed has loaded by then. */}
        <footer className="mt-6 flex flex-col items-center gap-2 text-[11px] text-slate-500 dark:text-white/30">
          <div className="flex items-center gap-2">
            {[
              { to: "/about", label: t("footer_about") || "About Us" },
              ...
              <span key={link.to} className="flex items-center gap-2">
                ...
                <Link
                  to={link.to}
                  className="hover:text-slate-700 dark:hover:text-white/60 hover:underline transition-colors"
                >
                  {link.label}
                </Link>
              </span>
          ...
          <div>
            ...
            <a
              href="https://humanrightspolicy.org/"
              target="_blank"
      ...
      {/* Public preview panel — world map + a slice of the social feed.
          Display-only: no comments/reactions/reports/share here, and
          map popups don't link to details — everything on this screen
          is just a public glance at the data, before login. A divider
          (a line above it when stacked on mobile, a line to its left
          side-by-side on desktop) visually separates it from the login
          card above/beside it. */}
      <div
        className="w-full max-w-xl mx-auto lg:mx-0 space-y-4
        border-t lg:border-t-0 lg:border-l border-slate-300 dark:border-white/[0.08]
        pt-8 lg:pt-0 lg:pl-10"
      >
        <PublicWorldMap />
        <PublicFeedPreview />
      </div>
