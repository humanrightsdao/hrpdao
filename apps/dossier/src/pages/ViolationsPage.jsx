...
const ViolationsPage = () => {
  ...
  // ADDED: the description textarea used to be a fixed 4 rows with
  // resize disabled — fine for a short note, but this field allows up
  // to MAX_VIOLATION_DESCRIPTION_LENGTH (6000) characters for a proper
  // detailed report, so 4 rows meant almost all of a long entry was
  // hidden below a tiny scrollable box while typing or proof-reading.
  // Now it starts taller and grows with the content (like the
  // quick-comment box in PostCard.jsx and the chat input in
  // NostrChatPage.jsx), up to a generous cap, and can still be
  // dragged even taller by hand since resize is no longer disabled.
  const violationDescriptionRef = useRef(null);
  const VIOLATION_DESCRIPTION_MAX_HEIGHT = 480;

  const autoResizeViolationDescription = useCallback(() => {
    const el = violationDescriptionRef.current;
    if (!el) return;
    el.style.height = "auto";
    const nextHeight = Math.min(
      el.scrollHeight,
      VIOLATION_DESCRIPTION_MAX_HEIGHT,
    );
              ...
              <div className="p-4">
                ...
                <textarea
                  ...
                  onChange={(e) => {
                    ...
                    autoResizeViolationDescription();
                  }}
                  placeholder={
                  ...
                  className={`w-full px-3 py-2 bg-slate-50 dark:bg-white/[0.03] border ${
                    validationErrors.violationDescription
                      ? "border-red-500/40"
                      : "border-slate-300 dark:border-white/[0.09]"
                  } rounded-lg text-[14px] text-slate-600 dark:text-white/65 placeholder-slate-300 dark:placeholder-white/[0.18] font-['Inter'] outline-none focus:border-blue-500/35 transition-all resize-y`}
                  style={{ maxHeight: `${VIOLATION_DESCRIPTION_MAX_HEIGHT}px` }}
                />
