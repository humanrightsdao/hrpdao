...
contract Deploy is Script {
    ...
    function _deployCore(DeployConfig memory cfg, address deployer) internal {
        ...
        // ⚠️ SEPOLIA_REHEARSAL_MOCK_HUMANITY — ЛИШЕ для фінальної репетиції
        // на Arbitrum Sepolia з TESTNET=false (реальний верифікатор, реальні
        // governance-затримки), коли решта конфігурації навмисно "як на
        // mainnet", але офіційного Human Passport Decoder на Arbitrum
        // Sepolia не існує (перевірено: задеплоєний лише на Arbitrum One,
        // Optimism, Optimism Sepolia, Linea, Scroll — не на Arbitrum
        // Sepolia). На mainnet цю змінну НІКОЛИ не задають — там
        // передається реальний PASSPORT_DECODER, а вимога нижче
        // (passportDecoder != address(0)) лишається обов'язковою.
        bool sepoliaRehearsalMockHumanity = vm.envOr("SEPOLIA_REHEARSAL_MOCK_HUMANITY", false);
        if (cfg.testMode || sepoliaRehearsalMockHumanity) {
            // ⚠️ ЛИШЕ testnet/репетиція: 100%-score мок-провайдер, щоб не
            // залежати від реального Human Passport Decoder під час
            // тестування.
            mockHumanityProvider = new MockHumanityProvider(10_000, deployer);
            humanityGate.addProvider(address(mockHumanityProvider));
        }
        require(
            cfg.passportDecoder != address(0) || cfg.testMode || sepoliaRehearsalMockHumanity,
    ...
    function _deploySBTs(DeployConfig memory cfg, address deployer) internal {
        ...
        shieldSBT = new ShieldSBT(
            ...
            cfg.testMode,
            keccak256("ipfs://QmVqW9rm7VC12piA9prsx1darf5xGfYwAEY1nkJfEoinuv"), // keccak256 від самого URI (не від тексту — той завеликий і багатомовний)
            "ipfs://QmVqW9rm7VC12piA9prsx1darf5xGfYwAEY1nkJfEoinuv"
        );
        if (cfg.testMode) {
