 ...
 *   2. Корінь публікується ЛИШЕ через TREASURER_ROLE (той самий
 *      DaoTimelock, що керує Treasury) — тобто публікація снепшоту
 *      проходить через ЗВИЧАЙНИЙ propose→vote→queue→execute цикл
 *      DaoGovernor, а не одноосібне рішення деплоєра/multisig.
