...
// ⚠️ Цей файл — GPL-3.0 (НЕ MIT, на відміну від решти проєкту), автоматично
// згенерований `snarkjs zkey export solidityverifier` з
// HexAncestry_plonk_real.zkey (сама схема без змін:
// zk/circuits/HexAncestry.circom). НЕ редагувати вручну — перегенерувати
// той самий файл заново після будь-якої зміни схеми чи trusted setup.
//
// ✅ РЕГЕНЕРОВАНО З РЕАЛЬНОГО ПУБЛІЧНОГО PTAU (2026-09-07)
// Ця версія zkey згенерована з powersOfTau28_hez_final_12.ptau —
// офіційний Hermez Powers-of-Tau (той самий universal SRS, що
// використовує Polygon Hermez zkEVM у проді, 54 реальні контрибутори +
// beacon), а НЕ з локального тестового ptau попередньої версії файлу.
// Перегенерація: snarkjs plonk setup zk/build/HexAncestry.r1cs
// powersOfTau28_hez_final_12.ptau HexAncestry_plonk_real.zkey, потім
// snarkjs zkey export solidityverifier HexAncestry_plonk_real.zkey.
//
// Перевірено end-to-end реальним доказом на реальному EVM (не лише
// компіляцією): валідний доказ (input.json → witness → plonk prove)
// приймається (`verifyProof()` → true), підроблений доказ відхиляється
// (`verifyProof()` → false). Деталі процедури — ZK_PRIVACY_V13_CHANGES.md,
// розділ "Як перевірити перед деплоєм".
//
// ⚠️ Повна крипто-верифікація транскрипту всієї 2^28-церемонії
// (snarkjs powersoftau verify) НЕ була прогнана під час цієї заміни —
// файл занадто важкий для перевірки за розумний час у звичайному
// середовищі розробки. Використано загальновідомий публічний файл
// напряму. Якщо потрібна повна ланцюгова верифікація before mainnet —
// прогнати `snarkjs powersoftau verify powersOfTau28_hez_final_12.ptau`
// окремо, у фоні, з достатнім часом (може займати години).
...
contract PlonkVerifier {
    ...
    uint256 constant Qmx  = 18828302590438805346654462084945167439236204454447759319640584145117268637042;
    uint256 constant Qmy  = 16476241591940834800042591510284476335269834882824808286032079519733599836397;
    uint256 constant Qlx  = 20912916196390819873227042545687909335514895739837963798376051527588508568716;
    uint256 constant Qly  = 7274736351563796003339191105667730211972901826856121731370010810344511553238;
    uint256 constant Qrx  = 19461650072800404312842450506213162239774215618674953603837700361011115258615;
    uint256 constant Qry  = 4976177118486025980391029723428282859695683312036587958413716768579375539413;
    uint256 constant Qox  = 7817163240592884996403195502985553968455562033751040676757737867658141938992;
    uint256 constant Qoy  = 10906883372799646838550578737305573529393485580668155652848438404439532467564;
    uint256 constant Qcx  = 6893214888021349366270614155483176841639269083173203443702401974379863576946;
    uint256 constant Qcy  = 19065700667576946144843492530855864471338025800790818769776025137697116985206;
    uint256 constant S1x  = 2300502299817661668082047901979759075164578617219141500947199195571655141570;
    uint256 constant S1y  = 7312514347087136708960744726141786093496889848431778200247803535859147242698;
    uint256 constant S2x  = 11114612292240549169937167793542364781472812659361049923907522288111156356652;
    uint256 constant S2y  = 14847381950488084976761664761517032066094136360786618441417027435346561116644;
    uint256 constant S3x  = 14616487405645449998627079570116341879025777700580660632271864144904018054325;
    uint256 constant S3y  = 6911624344083067800875779967088748064943200564928399006659808491708852115578;
