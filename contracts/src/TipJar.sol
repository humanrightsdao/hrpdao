 ...
 *      може бути змінений голосуванням у діапазоні [50%; 95%]
 *      (MIN_TAX_BPS/MAX_TAX_BPS) для кожного рівня незалежно — тобто
 *      автору голосуванням ДАО можна лише ЗМЕНШИТИ частку (з 50% до
 *      мінімум 5%), але не збільшити вище дефолтних 50%.
...
contract TipJar is AccessControl {
    ...
    /// @notice Межі "податку" (частки, що йде в Support Pool/ДАО) — 50% .. 95%, в bps.
    ///         Застосовується до ВСІХ рівнів (COUNCIL / SHIELD / NONE).
    ///         Автор ЗАВЖДИ отримує МАКСИМУМ 50% — голосуванням ДАО частку
    ///         пулу можна тільки збільшувати (до 95%), відповідно частка
    ///         автора може лише зменшуватись (до 5%), але ніколи не рости
    ///         вище дефолтних 50%.
    uint256 public constant MIN_TAX_BPS = 5_000; // 50% — мінімум для ДАО/пулу
    uint256 public constant MAX_TAX_BPS = 9_500; // 95% — максимум для ДАО/пулу

    /// @notice Поточні частки автора (bps). Податок = BPS_DENOMINATOR - authorBps.
    ///         Змінюються лише через голосування ДАО (DAO_ROLE), в межах
    ///         [MIN_TAX_BPS, MAX_TAX_BPS] для відповідного податку — тобто
    ///         сам authorBps завжди лежить у [BPS_DENOMINATOR-MAX_TAX_BPS,
    ///         BPS_DENOMINATOR-MIN_TAX_BPS] = [5%, 50%].
     ...
     *         і має лежати в межах [MIN_TAX_BPS; MAX_TAX_BPS] = [50%; 95%].
     *         Тобто частка автора завжди в межах [5%; 50%] — голосуванням
     *         ДАО її можна лише зменшити відносно дефолтних 50%.
    ...
    ///      податок (частка пулу/ДАО) лежить у дозволеному діапазоні [50%; 95%]
    ///      — тобто частка автора (authorBps) завжди в межах [5%; 50%].
    function _validateTax(uint256 authorBps) internal pure {
        ...
        require(taxBps >= MIN_TAX_BPS && taxBps <= MAX_TAX_BPS, "TipJar: tax out of [50%,95%] range");
    }
