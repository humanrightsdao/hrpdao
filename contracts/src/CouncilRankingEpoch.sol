...
contract CouncilRankingEpoch is AccessControl {
    ...
    /// @notice ГЕО-РЕФОРМА v15 ("живучий global-гейтинг"): номер останньої
    ///         епохи, для якої РЕАЛЬНО опубліковано ненульовий корінь
    ///         (submitSeatAssignments()/submitSeatRoot()). НЕ дорівнює
    ///         currentEpoch одразу після beginNewEpoch() — між стартом
    ///         нової епохи й публікацією її кореня завжди є розрив
    ///         (окремі транзакції, окремі кроки офчейн-обчислення).
    ///         DaoGovernor звіряє seat-докази проти ЦЬОГО номера, а не
    ///         currentEpoch — щоб перевірка завжди мала проти чого пройти,
    ///         навіть у розриві (ціна — дані можуть відставати щонайбільше
    ///         на один епохальний цикл, minEpochDuration).
    uint256 public lastSubmittedEpoch;

    /// @notice Мітка часу останньої публікації кореня (lastSubmittedEpoch).
    ///         Разом з DaoGovernor.maxRootStaleness — детермінований,
    ///         ончейн-перевірюваний "dead man's switch": якщо сабмітер
    ///         зникає надовго понад очікуваний цикл, гейтинг сам себе
    ///         тимчасово знімає, без ручного втручання guardian/DAO.
    ...
    function submitSeatAssignments(
        ...
        lastSubmittedEpoch = currentEpoch;
    ...
    function submitSeatRoot(bytes32 root, uint256 leafCount) external onlyRole(EPOCH_SUBMITTER_ROLE) {
        require(root != bytes32(0), "CouncilRanking: zero root");
        epochSeatRoot[currentEpoch] = root;
    ...
    /// @notice ГЕО-РЕФОРМА v15: чи "прострочені" дані ранжування —
    ///         минуло понад maxStaleness з моменту останньої публікації
    ///         кореня (lastSubmittedAt). Якщо кореня не було ЖОДНОГО разу
    ///         (lastSubmittedAt == 0) — вважається простроченим одразу
    ///         (це той самий бутстрап-кейс, що вже й так не гейтиться,
    ///         бо earthOverflowed == false до появи 244+ Консулів).
    ///         Викликається з DaoGovernor._propose() як частина
    ///         dead man's switch: сабмітер зникає надовго → гейтинг
    ///         тимчасово й прозоро знімається сам, без guardian/DAO.
    function isRootStale(uint256 maxStaleness) external view returns (bool) {
        if (lastSubmittedAt == 0) return true;
        return block.timestamp - lastSubmittedAt > maxStaleness;
    }
}
