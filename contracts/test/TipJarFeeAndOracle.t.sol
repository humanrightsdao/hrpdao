...
contract TipJarFeeAndOracleTest is Test {
    ...
    function test_setSplits_stillBoundedToFivePercentMinFiftyPercentMax() public {
        vm.startPrank(deployer);
        // Автору більше 50% — заборонено (податок ДАО був би <50%, нижче нового мінімуму).
        vm.expectRevert("TipJar: tax out of [50%,95%] range");
        tipJar.setSplits(5_100, 5_000, 5_000); // council-автор був би 51% — вище нового максимуму 50%

        // Автору менше 5% — заборонено (податок ДАО був би >95%, вище нового максимуму).
        vm.expectRevert("TipJar: tax out of [50%,95%] range");
        tipJar.setSplits(400, 5_000, 5_000); // council-автор був би 4% — нижче нового мінімуму 5%

        tipJar.setSplits(500, 3_000, 4_000); // автор 5%, 30%, 40% — усе в межах [5%,50%]
        vm.stopPrank();

        assertEq(tipJar.councilAuthorBps(), 500);
        assertEq(tipJar.shieldAuthorBps(), 3_000);
        assertEq(tipJar.noneAuthorBps(), 4_000);
    }
}
