...
contract ScopedProposalsTest is Test {
    ...
    function test_proposeScoped_earthAlwaysAllowed() public {
        ...
        // ГЕО-РЕФОРМА v15: seat-перевірка для EARTH переїхала з
        // _validateScopeChoice() у централізований гейтинг _propose()
        // (однаковий і для proposeScoped(EARTH,...), і для звичайної
        // propose() — див. GEO_REFORM_V15_CHANGES.md). Тепер її треба
        // застовпити ОКРЕМО, один раз на епоху, через claimGlobalSeat() —
        // proposeScoped() більше не приймає доказ напряму для EARTH.
        vm.prank(bobInHexA);
        daoGovernor.claimGlobalSeat(0, new bytes32[](0));
    ...
    function test_earthScopedProposal_anyoneCanVote() public {
        ...
        // ГЕО-РЕФОРМА v15: див. пояснення в test_proposeScoped_earthAlwaysAllowed.
        vm.prank(bobInHexA);
        daoGovernor.claimGlobalSeat(0, new bytes32[](0));
    ...
    ///         ГЕО-РЕФОРМА v15: сама перевірка переїхала з
    ///         _validateScopeChoice() у централізований гейтинг _propose()
    ///         (щоб та сама перевірка діяла і для звичайної propose(),
    ///         яку раніше можна було просто оминути) — звідси нове
    ///         повідомлення ревёрту, що вказує на claimGlobalSeat().
    function test_proposeScoped_earthRevertsWithoutSeat_whenOverflowed() public {
        ...
        vm.prank(daveInHexB);
        vm.expectRevert("DaoGovernor: claim your EARTH seat first (claimGlobalSeat)");
    ...
    /// @notice ⚠️ РЕГРЕСІЙНИЙ ТЕСТ НА САМУ ЗНАХІДКУ (ГЕО-РЕФОРМА v15):
    ///         раніше звичайна propose() не мала ЖОДНОЇ рейтингової
    ///         перевірки, тому будь-хто міг просто оминути весь
    ///         "EARTH-гейтинг" в proposeScoped(), викликавши propose()
    ///         напряму. daveInHexB тут НЕ має EARTH seat (як і в
    ///         test_proposeScoped_earthRevertsWithoutSeat_whenOverflowed) —
    ///         до фіксу цей виклик проходив би без жодних перешкод.
    function test_plainPropose_nowGatedSameAsEarthScoped_whenOverflowed() public {
        (address[] memory t, uint256[] memory v, bytes[] memory c) = _emptyCalldata();

        vm.prank(daveInHexB);
        vm.expectRevert("DaoGovernor: claim your EARTH seat first (claimGlobalSeat)");
        daoGovernor.propose(t, v, c, "bypass attempt via plain propose()");
    }

    /// @notice Після claimGlobalSeat() звичайна propose() відкривається —
    ///         підтверджує, що це не повна заборона, а справжній claim-крок.
    function test_plainPropose_worksAfterClaimGlobalSeat() public {
        (address[] memory t, uint256[] memory v, bytes[] memory c) = _emptyCalldata();

        // daveInHexB отримує власне EARTH-місце (окремо від bobInHexA з
        // setUp) — дерево з двома листками: [bob(EARTH), dave(EARTH)].
        address[] memory seatAccounts = new address[](2);
        bool[] memory seatFlags = new bool[](2);
        int8[] memory seatLevels = new int8[](2);
        uint64[] memory seatBranches = new uint64[](2);
        seatAccounts[0] = bobInHexA;   seatFlags[0] = true; seatLevels[0] = rankingEpoch.LEVEL_EARTH(); seatBranches[0] = rankingEpoch.EARTH_BRANCH();
        seatAccounts[1] = daveInHexB;  seatFlags[1] = true; seatLevels[1] = rankingEpoch.LEVEL_EARTH(); seatBranches[1] = rankingEpoch.EARTH_BRANCH();
        vm.prank(deployer);
        rankingEpoch.submitSeatAssignments(seatAccounts, seatFlags, seatLevels, seatBranches);

        // Дерево з 2 листків: корінь = commutativeKeccak256(leaf0, leaf1).
        // Доказ для листка з index=1 (dave) — це сусідній листок (index=0,
        // bob), а НЕ порожній масив — з порожнім proof MerkleProof.verify
        // звіряє сам leaf як корінь, що не збігається зі збереженим коренем.
        bytes32[] memory proof = new bytes32[](1);
        proof[0] = rankingEpoch.leafHash(0, bobInHexA, true, rankingEpoch.LEVEL_EARTH(), rankingEpoch.EARTH_BRANCH());

        vm.prank(daveInHexB);
        daoGovernor.claimGlobalSeat(1, proof);

        vm.prank(daveInHexB);
        uint256 id = daoGovernor.propose(t, v, c, "now allowed after claim");

        (, , bool set) = daoGovernor.proposalScope(id);
        assertFalse(set); // звичайна propose() — без scope, на відміну від proposeScoped
    }

    /// @notice claimGlobalSeat() з невалідним доказом (справжнього місця
    ///         немає) має ревертати — claim не можна підробити.
    function test_claimGlobalSeat_revertsWithInvalidProof() public {
        // daveInHexB НЕ входить у дерево, поданe в setUp (там лише bob) —
        // будь-який seatIndex/порожній доказ для нього невалідний.
        vm.prank(daveInHexB);
    ...
    ///         кореня минуло більше maxRootStaleness (дефолт 30 днів) —
    ///         гейтинг сам, без жодного claim, тимчасово знімається.
    function test_plainPropose_allowedWithoutClaim_whenRootStale() public {
        (address[] memory t, uint256[] memory v, bytes[] memory c) = _emptyCalldata();

        // Переконуємось, що без перемотування часу шлях і далі заблокований
        // (щоб тест дійсно перевіряв ЕФЕКТ staleness, а не щось інше).
        vm.prank(daveInHexB);
        vm.expectRevert("DaoGovernor: claim your EARTH seat first (claimGlobalSeat)");
        daoGovernor.propose(t, v, c, "too early");

        // Останній корінь був опублікований у setUp (t=0) — перемотуємо
        // за межі maxRootStaleness (дефолт 30 днів).
        vm.warp(block.timestamp + daoGovernor.maxRootStaleness() + 1);

        vm.prank(daveInHexB);
        uint256 id = daoGovernor.propose(t, v, c, "allowed via stale fallback");
        assertTrue(id != 0);
    }

    function test_setMaxRootStaleness_onlyViaGovernance() public {
        vm.expectRevert("DaoGovernor: only via governance");
        daoGovernor.setMaxRootStaleness(7 days);

        vm.prank(address(daoTimelock));
        daoGovernor.setMaxRootStaleness(7 days);
        assertEq(daoGovernor.maxRootStaleness(), 7 days);
    }

    function test_setMaxRootStaleness_revertsOnZero() public {
        vm.prank(address(daoTimelock));
        vm.expectRevert("DaoGovernor: zero staleness");
        daoGovernor.setMaxRootStaleness(0);
    }
}
